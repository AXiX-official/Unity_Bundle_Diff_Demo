...
namespace UnityAsset.NET.IO.Reader
{
    ...
    public class BlockReader : IReader
    {
        ...
        private byte[] DecompressBlock(int index)
        {
            ...
            try
            {
                File.ReadExactly(compressedBuffer, 0, block.CompressedSize);
                ...
                try
                {
                    ...
                    {
                        Compression.Compression.DecompressToBytes(compressedSpan, uncompressedSpan, block.CompressionType);
                    }
