...
namespace Editor
{
    public class DiffGenerator
    {
        ...
        public async Task<PatchManifest> GenerateAsync(string version, string baseVersion, IProgress<LoadProgress>? progress = null)
        {
            Directory.CreateDirectory(Path.Combine(_outputDir, "patches"));

            var oldMgr = new AssetManager();
            await oldMgr.LoadDirectoryAsync(_oldDir, progress: progress);

            var newMgr = new AssetManager();
            await newMgr.LoadDirectoryAsync(_newDir, progress: progress);
            ...
            await File.WriteAllTextAsync(Path.Combine(_outputDir, "manifest.json"), json);
